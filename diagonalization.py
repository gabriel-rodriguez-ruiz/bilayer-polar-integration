...
def get_Hamiltonian_in_polars(k, theta, mu_s, mu_S, B_x_s, B_y_s,
    ...
    H_s = ( chi_k_s(k_x + phi_x - q_x, k_y + phi_y - q_y ) * np.kron(( tau_z + tau_0 )/2, sigma_0)
            - chi_k_s(-k_x + phi_x - q_x, -k_y + phi_y - q_y) * np.kron( (tau_z - tau_0 )/2, sigma_0)
            - B_y_s * np.kron(tau_0, sigma_y)
            - B_x_s * np.kron(tau_0, sigma_x)
            + Lambda * (k_x + phi_x - q_x) * np.kron( ( tau_z + tau_0 )/2, sigma_y )
            - Lambda * (-k_x + phi_x - q_x) * np.kron( (tau_z - tau_0 )/2, sigma_y )
            - Lambda * (k_y + phi_y - q_y) * np.kron( ( tau_z + tau_0 )/2, sigma_x )
            + Lambda * (-k_y + phi_y - q_y) * np.kron( ( tau_z - tau_0 )/2, sigma_x )
           )
...
def get_Energies_in_polars(k_values, theta_values, mu_s, mu_S, B_y, Delta,
                           phi_x, gamma, Lambda, B_x, phi_y):
    """Return the energies of the Hamiltonian at a given k."""
    E = np.zeros((len(k_values), len(theta_values), 4))
    for i, k in enumerate(k_values):
        for j, theta in enumerate(theta_values):
            H = get_Hamiltonian_in_polars(k, theta, t_s, t_S, t_c, mu_s, mu_S, B_x_s, B_y_s,
                                          B_x_S, B_y_S, Delta, phi_x, phi_y, q_x, q_y,
                                          gamma_s, gamma_S, Lambda)
